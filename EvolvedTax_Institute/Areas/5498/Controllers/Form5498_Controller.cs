...
{
    [Area("5498")]
    public class Form5498_Controller : BaseController
    {
        private readonly IForm5498_Service _form5498_Service;
        private readonly IInstituteService _instituteService;
        private readonly ICommonService _commonService;
        private readonly IWebHostEnvironment _webHostEnvironment;
        public Form5498_Controller(IForm5498_Service form5498_Service, IWebHostEnvironment webHostEnvironment, ICommonService commonService, IInstituteService instituteService)
        {
            _form5498_Service = form5498_Service;
        ...
        public IActionResult Index()
        {
            ...
            return View(_form5498_Service.GetForm5498List().Where(p => p.EntityId == EntityId));
        }
        [Route("Form5498_/uploadClients")]
        [HttpPost]
        public async Task<IActionResult> UploadClients(IFormFile file, int EntityId, string entityName)
        {
            ...
            var response = await _form5498_Service.Upload5498_Data(file, SessionUser.InstituteId, EntityId, SessionUser.UserId);
        ...
        public IActionResult ChangeEntity(int entityId)
        {
            ...
            var response = _form5498_Service.GetForm5498List().Where(p => p.EntityId == entityId);
        ...
        {
            string TemplatePathFile = Path.Combine(_webHostEnvironment.WebRootPath, "Forms", AppConstants.Form5498TemplateFileName);
            string SavePathFolder = Path.Combine(_webHostEnvironment.WebRootPath, AppConstants.Form5498);
            var entityId = HttpContext.Session.GetInt32("EntityId") ?? 0;
            string pdfUrl = _form5498_Service.GeneratePdf(Id, TemplatePathFile, SavePathFolder, entityId);
            return Json(pdfUrl);
        }

        [HttpPost]

        [Route("Form5498_/DownloadAll")]
        public IActionResult DownloadAll([FromBody] DownloadRequestModel model)
        {
            ...
            string SavePathFolder = Path.Combine(_webHostEnvironment.WebRootPath, AppConstants.Form5498);
            var entityId = HttpContext.Session.GetInt32("EntityId") ?? 0;
            var zipFilePath = _form5498_Service.GenerateAndZipPdfs(ids, SavePathFolder, selectedPage, RootPath, entityId);
        ...
        [HttpPost]
        [Route("Form5498_/DownloadOneFile")]
        public IActionResult DownloadOneFile([FromBody] DownloadRequestModel model)
        {
            ...
            //}
            var entityId = HttpContext.Session.GetInt32("EntityId") ?? 0;
            var zipFilePath = _form5498_Service.DownloadOneFile(ids, SavePathFolder, selectedPage, RootPath, entityId);
            string contentType = "application/zip";
        ...
        public async Task<IActionResult> KeepRecord(int id)
        {
            ...
            var response = await _form5498_Service.KeepRecord(id);
            return Json(response);
        }
        ...
        public async Task<IActionResult> DeleteRecord(int id)
        {
            ...
            var response = await _form5498_Service.DeletePermeant(id);
            return Json(response);
        }
    }
}
