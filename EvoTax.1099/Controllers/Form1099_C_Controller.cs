...
{
    public class Form1099_C_Controller : BaseController
    {
        private readonly IForm1099_C_Service _form1099_C_Service;
        private readonly IInstituteService _instituteService;
        private readonly IWebHostEnvironment _webHostEnvironment;
        public Form1099_C_Controller(IForm1099_C_Service Form1099_C_Service, IWebHostEnvironment webHostEnvironment, IInstituteService instituteService)
        {
        ...
        public IActionResult Index()
        {
            ...
            return View(_form1099_C_Service.GetForm1099List().Where(p => p.EntityId == EntityId && p.InstID == InstId));
        }
        [Route("Form1099_C_/uploadClients")]
        [HttpPost]
        public async Task<IActionResult> UploadClients(IFormFile file, int EntityId, string entityName)
        {
            ...
            var response = await _form1099_C_Service.Upload1099_Data(file, EntityId, SessionUser.InstituteId, SessionUser.UserId);
        ...
        {
            string TemplatePathFile = Path.Combine(_webHostEnvironment.WebRootPath, "Forms", AppConstants.C_1099_TemplateFileName);
            string SavePathFolder = Path.Combine(_webHostEnvironment.WebRootPath, "1099C");
            string pdfUrl = _form1099_C_Service.GeneratePdf(id, TemplatePathFile, SavePathFolder);
            return Json(pdfUrl);
        ...
        public IActionResult DownloadAll([FromBody] DownloadRequestModel model)
        {
            ...
            string SavePathFolder = Path.Combine(_webHostEnvironment.WebRootPath, "1099C");
            var zipFilePath = _form1099_C_Service.GenerateAndZipPdfs(ids, SavePathFolder, selectedPage, RootPath);
        ...
        [HttpPost]
        [Route("Form1099_C_/DownloadOneFile")]
        public IActionResult DownloadOneFile([FromBody] DownloadRequestModel model)
        {
            ...
            //    selectedPage.Add("7");
            //}
            var zipFilePath = _form1099_C_Service.DownloadOneFile(ids, SavePathFolder, selectedPage, RootPath);
            string contentType = "application/zip";
        ...
        public async Task<IActionResult> KeepRecord(int id)
        {
            ...
            var response = await _form1099_C_Service.KeepRecord(id);
            return Json(response);
        }

        [Route("Form1099_C_/DeleteRecord")]
        [HttpPost]
        public async Task<IActionResult> DeleteRecord(int id)
        {
            ...
            var response = await _form1099_C_Service.DeletePermeant(id);
            return Json(response);
        }
    }
}
