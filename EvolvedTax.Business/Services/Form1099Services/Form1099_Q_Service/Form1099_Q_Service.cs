...
namespace EvolvedTax.Business.Services.Form1099Services
{
    public class Form1099_Q_Service : IForm1099_Q_Service
    {
            ...
            using (var stream = file.OpenReadStream())
            {
                ...
                for (int row = 1; row <= sheet.LastRowNum; row++) // Starting from the second row
                {
                    ...
                    var request = new Tbl1099_Q
                    {
                        ...
                        Box_5_Checkbox1 = (excelRow.GetCell(columnMapping["Box 5 Checkbox 1"])?.ToString() != null && (bool)excelRow.GetCell(columnMapping["Box 5 Checkbox 1"])?.ToString().Equals("Yes", StringComparison.OrdinalIgnoreCase)) ? "1" : "0",
                        Box_5_Checkbox2 = (excelRow.GetCell(columnMapping["Box 5 Checkbox 2"])?.ToString() != null && (bool)excelRow.GetCell(columnMapping["Box 5 Checkbox 2"])?.ToString().Equals("Yes", StringComparison.OrdinalIgnoreCase)) ? "1" : "0",
                        Box_5_Checkbox3 = (excelRow.GetCell(columnMapping["Box 5 Checkbox 3"])?.ToString() != null && (bool)excelRow.GetCell(columnMapping["Box 5 Checkbox 3"])?.ToString().Equals("Yes", StringComparison.OrdinalIgnoreCase)) ? "1" : "0",
                        Box_6_Checkbox = (excelRow.GetCell(columnMapping["Box 6 Checkbox"])?.ToString() != null && (bool)excelRow.GetCell(columnMapping["Box 6 Checkbox"])?.ToString().Equals("Yes", StringComparison.OrdinalIgnoreCase)) ? "1" : "0",
                        Box_X_Amount = excelRow.GetCell(columnMapping["Box X Amount"])?.ToString() != null ? Convert.ToDecimal(excelRow.GetCell(columnMapping["Box X Amount"])?.ToString()) : 0,
                        Box_X_Code = excelRow.GetCell(columnMapping["Box X Code"])?.ToString() != null ? Convert.ToInt32(excelRow.GetCell(columnMapping["Box X Code"])?.ToString()) : 0,
        ...
        public string CreatePdf(int Id, string TemplatefilePath, string SaveFolderPath, bool IsAll, string Page = "")
        {
            ...
            var requestInstitue = _instituteService.GetPayeeData((int)Qresponse.InstID);
            ...
            if (Qresponse.Country != "United States")
            {
                var country = _evolvedtaxContext.MstrCountries.FirstOrDefault(c => c.Country == Qresponse.Country);
                if (country != null)
                {
            ...
            string RecipentCity = string.Join(", ",
               new[]
               {
                    Qresponse.City,
                    Qresponse.State,
                    string.IsNullOrWhiteSpace(Qresponse.Province) ? null : Qresponse.Province,
                     string.IsNullOrWhiteSpace(Recepient_CountryCode) ? null : Recepient_CountryCode,
                    Qresponse.Zip,
                    string.IsNullOrWhiteSpace(Qresponse.PostalCode) ? null : Qresponse.PostalCode

               }.Where(s => !string.IsNullOrWhiteSpace(s))
           );

            String RecipentAddress = string.Concat(Qresponse.Address_Deliv_Street, ", ", Qresponse.Address_Apt_Suite);
            ...
            pdfFormFields.SetField("topmostSubform.CopyB.LeftColumn.f1_2", requestInstitue.PayeeData);   //PayData

            pdfFormFields.SetField("topmostSubform.CopyB.LeftColumn.f1_3", requestInstitue.Idnumber);   //requestInstitue.Idnumber
            ...
            pdfFormFields.SetField("topmostSubform.CopyC.LeftColumn.f1_2", requestInstitue.PayeeData);   //PayData

            pdfFormFields.SetField("topmostSubform.CopyC.LeftColumn.f1_3", requestInstitue.Idnumber);   //requestInstitue.Idnumber
