...
namespace EvolvedTax.Business.Services.Form1099Services
{
    public class Form1099_CAP_Service : IForm1099_CAP_Service
    {
        ...
        public Form1099_CAP_Service(EvolvedtaxContext evolvedtaxContext, IMailService mailService, ITrailAudit1099Service trailAudit1099Service)
        {
            ...
            using (var stream = file.OpenReadStream())
            {
                ...
                for (int row = 1; row <= sheet.LastRowNum; row++) // Starting from the second row
                {
                    ...
                    var entity = new Tbl1099_CAP
                    {
                        ...
                        Box_3_Number = TryConvertToInt(excelRow.GetCell(15)),
                        Box_4_Class = excelRow.GetCell(16)?.ToString(),
                        Box_5_All_Lines = excelRow.GetCell(17)?.ToString(),
                        Form_Category = excelRow.GetCell(18)?.ToString(),
                        Form_Source = excelRow.GetCell(19)?.ToString(),
                        Tax_State = excelRow.GetCell(20)?.ToString(),
                        InstID = InstId,
                        EntityId = entityId,
                        //UserId = UserId,
