...
{
    public class Form3921Service : IForm3921Service
    {
        ...
        public Form3921Service(EvolvedtaxContext evolvedtaxContext, IMapper mapper, IInstituteService instituteService, IMailService mailService = null, ITrailAudit1099Service trailAudit1099Service = null)
        {
        ...
        public IQueryable<Tbl_3921> GetForm3921List()
        {
            var response = _evolvedtaxContext.Tbl_3921;
            return response;
        }
        public async Task<bool> SendEmailToRecipients(int[] selectValues, string URL, string form, int instituteId = -1)
        {
            var result = from ic in _evolvedtaxContext.Tbl_3921
        ...
        public async Task<MessageResponseModel> Upload3921Data(IFormFile file, int InstId, int entityId, string UserId)
        {
            bool Status = false;
            var response = new List<Tbl_3921>();
            var AList = new List<Tbl_3921>();
            using (var stream = file.OpenReadStream())
            {
                ...
                for (int row = 1; row <= sheet.LastRowNum; row++) // Starting from the second row
                {
                    ...
                    var request = new Tbl_3921
                    {
                        ...
                        Box6AllLines = excelRow.GetCell(columnMapping["Box6AllLines"])?.ToString() ?? string.Empty,
                        ...
                        PostalCode = excelRow.GetCell(columnMapping["PostalCode"])?.ToString() ?? string.Empty,
                        ...
                        Status = Convert.ToInt32(excelRow.GetCell(columnMapping["Status"])?.ToString() ?? "0"),
                        Box3Amount = Convert.ToInt32(excelRow.GetCell(columnMapping["Box3Amount"])?.ToString() ?? "0"),
                        Box4Amount = Convert.ToInt32(excelRow.GetCell(columnMapping["Box4Amount"])?.ToString() ?? "0"),
                        Box5Number = Convert.ToInt32(excelRow.GetCell(columnMapping["Box5Number"])?.ToString() ?? "0"),
                        ...
                        FormSource = excelRow.GetCell(columnMapping["FormSource"])?.ToString() ?? string.Empty,
                    ...
                    if (await _evolvedtaxContext.Tbl_3921.AnyAsync(p => p.RcpTIN == request.RcpTIN && p.EntityId == request.EntityId && p.Created_Date != null &&
                ...
                await _evolvedtaxContext.Tbl_3921.AddRangeAsync(AList);
        ...
        public string CreatePdf(int Id, string TemplatefilePath, string SaveFolderPath, int entityId, bool IsAll, string Page = "")
        {
            ...
            {
                newFile1 = string.Concat(ClientName, "_", AppConstants.Form3921, "_", response.Id, "_Page_", Page);
            }
            else
            {
                newFile1 = string.Concat(ClientName, "_", AppConstants.Form3921, "_", response.Id);
            }
        ...
        public string GeneratePdfForSpecificPage(int Id, string TemplatefilePath, string SaveFolderPath, List<string> selectedPages, int entityId)
        {
            ...
            newFile1 = string.Concat(ClientName, "_", AppConstants.Form3921, "_", Id);
            string FilenameNew = "/Form3921/" + newFile1 + ".pdf";
        ...
        public string GeneratePdForSpecificType(int Id, string TemplatefilePath, string SaveFolderPath, string selectedPage, int entityId = 0)
        {
            ...
            newFile1 = string.Concat(ClientName, "_", AppConstants.Form3921, "_", request.Id, "_Page_", selectedPage);
            string FilenameNew = "/Form3921/" + newFile1 + ".pdf";
            string newFileName = newFile1 + ".pdf";
        ...
        public string DownloadOneFile(List<int> ids, string SaveFolderPath, List<string> selectedPages, string RootPath, int entityId = 0)
        {
            ...
            string TemplatePathFile = Path.Combine(RootPath, "Forms", AppConstants.Form3921TemplateFileName);
        ...
        public async Task<MessageResponseModel> DeletePermeant(int id)
        {
            ...
            var recordToDelete = _evolvedtaxContext.Tbl_3921.First(p => p.Id == id);
            if (recordToDelete != null)
            {
                _evolvedtaxContext.Tbl_3921.Remove(recordToDelete);
                await _evolvedtaxContext.SaveChangesAsync();
        ...
        public async Task<MessageResponseModel> KeepRecord(int id)
        {
            ...
            var recordToUpdate = _evolvedtaxContext.Tbl_3921.First(p => p.Id == id);
