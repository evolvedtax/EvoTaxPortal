...
{
    public class Form1098_C_Service : IForm1098_C_Service
    {
        ...
        public Form1098_C_Service(EvolvedtaxContext evolvedtaxContext, IMapper mapper, IInstituteService instituteService, IMailService mailService = null, ITrailAudit1099Service trailAudit1099Service = null)
        {
        ...
        public IQueryable<Form1098CResponse> GetForm1098CList()
        {
            var response = _evolvedtaxContext.Tbl_1098_C.Select(p => new Form1098CResponse
            {
                Id = p.Id,
                RcpTIN = p.RcpTIN,
                ...
                RcpEmail = p.RcpEmail,
                Box1Date = p.Box1Date,
                Box2aMileage = p.Box2aMileage,
                Box2bYear = p.Box2bYear,
                Box2cMake = p.Box2cMake,
                Box2dModel = p.Box2dModel,
                Box3IDNum = p.Box3IDNum,
                Box4aCheckbox = p.Box4aCheckbox,
                Box4bDate = p.Box4bDate,
                Box4cAmount = p.Box4cAmount,
                Box5aCheckbox = p.Box5aCheckbox,
                Box5bCheckbox = p.Box5bCheckbox,
                Box5cAllLines = p.Box5cAllLines,
                Box6aCheckbox = p.Box6aCheckbox,
                Box6bAmount = p.Box6bAmount,
                Box6cCheckbox = p.Box6cCheckbox,
                Box6cAllLines = p.Box6cAllLines,
        ...
        public async Task<MessageResponseModel> Upload1098_C_Data(IFormFile file, int InstId, int entityId, string UserId)
        {
            bool Status = false;
            var response = new List<Tbl_1098_C>();
            var AList = new List<Tbl_1098_C>();
            using (var stream = file.OpenReadStream())
            {
                ...
                for (int row = 1; row <= sheet.LastRowNum; row++) // Starting from the second row
                {
                    ...
                    var request = new Tbl_1098_C
                    {
                        ...
                        Box2aMileage = Convert.ToInt32(excelRow.GetCell(columnMapping["Box2aMileage"])?.ToString() ?? "0"),
                        Box2bYear = Convert.ToInt32(excelRow.GetCell(columnMapping["Box2bYear"])?.ToString() ?? "0"),
                        Box2cMake = excelRow.GetCell(columnMapping["Box2cMake"])?.ToString() ?? string.Empty,
                        Box2dModel = excelRow.GetCell(columnMapping["Box2dModel"])?.ToString() ?? string.Empty,
                        Box3IDNum = excelRow.GetCell(columnMapping["Box3IDNum"])?.ToString() ?? string.Empty,
                        Box4aCheckbox = (excelRow.GetCell(columnMapping["Box4aCheckbox"])?.ToString() != null && (bool)excelRow.GetCell(columnMapping["Box4aCheckbox"])?.ToString().Equals("Yes", StringComparison.OrdinalIgnoreCase)) ? "1" : "0",
                        Box4bDate = excelRow.GetCell(columnMapping["Box4bDate"])?.ToString() == null ? null : Convert.ToDateTime(excelRow.GetCell(columnMapping["Box4bDate"]).ToString()),
                        Box4cAmount = Convert.ToDecimal(excelRow.GetCell(columnMapping["Box4cAmount"])?.ToString() ?? "0.0"),
                        Box5aCheckbox = (excelRow.GetCell(columnMapping["Box5aCheckbox"])?.ToString() != null && (bool)excelRow.GetCell(columnMapping["Box5aCheckbox"])?.ToString().Equals("Yes", StringComparison.OrdinalIgnoreCase)) ? "1" : "0",
                        Box5bCheckbox = (excelRow.GetCell(columnMapping["Box5bCheckbox"])?.ToString() != null && (bool)excelRow.GetCell(columnMapping["Box5bCheckbox"])?.ToString().Equals("Yes", StringComparison.OrdinalIgnoreCase)) ? "1" : "0",
                        Box5cAllLines = excelRow.GetCell(columnMapping["Box5cAllLines"])?.ToString() ?? string.Empty,
                        Box6aCheckbox = (excelRow.GetCell(columnMapping["Box6aCheckbox"])?.ToString() != null && (bool)excelRow.GetCell(columnMapping["Box6aCheckbox"])?.ToString().Equals("Yes", StringComparison.OrdinalIgnoreCase)) ? "1" : "0",
                        Box6bAmount = Convert.ToDecimal(excelRow.GetCell(columnMapping["Box6bAmount"])?.ToString() ?? "0.0"),
                        Box6cCheckbox = (excelRow.GetCell(columnMapping["Box6cCheckbox"])?.ToString() != null && (bool)excelRow.GetCell(columnMapping["Box6cCheckbox"])?.ToString().Equals("Yes", StringComparison.OrdinalIgnoreCase)) ? "1" : "0",
                        Box6cAllLines = excelRow.GetCell(columnMapping["Box6cAllLines"])?.ToString() ?? string.Empty,
                    ...
                    if (await _evolvedtaxContext.Tbl_1098_C.AnyAsync(p => p.RcpTIN == request.RcpTIN && p.EntityId == request.EntityId && p.Created_Date != null &&
                ...
                await _evolvedtaxContext.Tbl_1098_C.AddRangeAsync(AList);
        ...
        public string CreatePdf(int Id, string TemplatefilePath, string SaveFolderPath, int entityId, bool IsAll, string Page = "")
        {
            ...
            {
                newFile1 = string.Concat(ClientName, "_", AppConstants.Form1098C, "_", response.Id, "_Page_", Page);
            }
            else
            {
                newFile1 = string.Concat(ClientName, "_", AppConstants.Form1098C, "_", response.Id);
            }


            string FilenameNew = "/Form1098C/" + newFile1 + ".pdf";
        ...
        public string GeneratePdfForSpecificPage(int Id, string TemplatefilePath, string SaveFolderPath, List<string> selectedPages, int entityId)
        {
            ...
            newFile1 = string.Concat(ClientName, "_", AppConstants.Form1098C, "_", Id);
            string FilenameNew = "/Form1098C/" + newFile1 + ".pdf";
        ...
        public string GeneratePdForSpecificType(int Id, string TemplatefilePath, string SaveFolderPath, string selectedPage, int entityId = 0)
        {
            ...
            newFile1 = string.Concat(ClientName, "_", AppConstants.Form1098C, "_", request.Id, "_Page_", selectedPage);
            string FilenameNew = "/Form1098C/" + newFile1 + ".pdf";
            string newFileName = newFile1 + ".pdf";
        ...
        public string DownloadOneFile(List<int> ids, string SaveFolderPath, List<string> selectedPages, string RootPath, int entityId = 0)
        {
            ...
            string TemplatePathFile = Path.Combine(RootPath, "Forms", AppConstants.Form1098CTemplateFileName);
        ...
        public async Task<MessageResponseModel> DeletePermeant(int id)
        {
            ...
            var recordToDelete = _evolvedtaxContext.Tbl_1098_C.First(p => p.Id == id);
            if (recordToDelete != null)
            {
                _evolvedtaxContext.Tbl_1098_C.Remove(recordToDelete);
                await _evolvedtaxContext.SaveChangesAsync();
        ...
        public async Task<MessageResponseModel> KeepRecord(int id)
        {
            ...
            var recordToUpdate = _evolvedtaxContext.Tbl_1098_C.First(p => p.Id == id);
