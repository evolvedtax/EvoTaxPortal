...
{
    public class Form1098_F_Service : IForm1098_F_Service
    {
        ...
        public Form1098_F_Service(EvolvedtaxContext evolvedtaxContext, IMapper mapper, IInstituteService instituteService, IMailService mailService = null, ITrailAudit1099Service trailAudit1099Service = null)
        {
        ...
        public IQueryable<Form1098FResponse> GetForm1098FList()
        {
            var response = _evolvedtaxContext.Tbl_1098_F.Select(p => new Form1098FResponse
            {
                Id = p.Id,
                RcpTIN = p.RcpTIN,
                ...
                Box5Date = p.Box5Date,
                Box6CourtEntity = p.Box6CourtEntity,
                Box7CaseNumber = p.Box7CaseNumber,
                Box8CaseParties = p.Box8CaseParties,
                Box9AllCodes = p.Box9AllCodes,
        ...
        {
            var result = from ic in _evolvedtaxContext.Tbl_1098_F
                         where selectValues.Contains(ic.Id) && !string.IsNullOrEmpty(ic.RcpEmail)
                         select new Tbl_1098_F
                         {
        ...
        public async Task<MessageResponseModel> Upload1098_F_Data(IFormFile file, int InstId, int entityId, string UserId)
        {
            bool Status = false;
            var response = new List<Tbl_1098_F>();
            var AList = new List<Tbl_1098_F>();
            using (var stream = file.OpenReadStream())
            {
                ...
                for (int row = 1; row <= sheet.LastRowNum; row++) // Starting from the second row
                {
                    ...
                    var request = new Tbl_1098_F
                    {
                        ...
                        Box5Date = excelRow.GetCell(columnMapping["Box5Date"])?.ToString() == null ? null : Convert.ToDateTime(excelRow.GetCell(columnMapping["Box5Date"]).ToString()),
                        Box6CourtEntity = excelRow.GetCell(columnMapping["Box6CourtEntity"])?.ToString() ?? string.Empty,
                        Box7CaseNumber = excelRow.GetCell(columnMapping["Box7CaseNumber"])?.ToString() ?? string.Empty,
                        Box8CaseParties = excelRow.GetCell(columnMapping["Box8CaseParties"])?.ToString() ?? string.Empty,
                        Box9AllCodes = excelRow.GetCell(columnMapping["Box9AllCodes"])?.ToString() ?? string.Empty,
                    ...
                    if (await _evolvedtaxContext.Tbl_1098_F.AnyAsync(p => p.RcpTIN == request.RcpTIN && p.EntityId == request.EntityId && p.Created_Date != null &&
                ...
                await _evolvedtaxContext.Tbl_1098_F.AddRangeAsync(AList);
        ...
        public string CreatePdf(int Id, string TemplatefilePath, string SaveFolderPath, int entityId, bool IsAll, string Page = "")
        {
            ...
            {
                newFile1 = string.Concat(ClientName, "_", AppConstants.Form1098F, "_", response.Id, "_Page_", Page);
            }
            else
            {
                newFile1 = string.Concat(ClientName, "_", AppConstants.Form1098F, "_", response.Id);
            }


            string FilenameNew = "/Form1098T/" + newFile1 + ".pdf";
        ...
        public string GeneratePdfForSpecificPage(int Id, string TemplatefilePath, string SaveFolderPath, List<string> selectedPages, int entityId)
        {
            ...
            newFile1 = string.Concat(ClientName, "_", AppConstants.Form1098F, "_", Id);
            string FilenameNew = "/Form1098T/" + newFile1 + ".pdf";
        ...
        public string GeneratePdForSpecificType(int Id, string TemplatefilePath, string SaveFolderPath, string selectedPage, int entityId = 0)
        {
            ...
            newFile1 = string.Concat(ClientName, "_", AppConstants.Form1098F, "_", request.Id, "_Page_", selectedPage);
            string FilenameNew = "/Form1098T/" + newFile1 + ".pdf";
            string newFileName = newFile1 + ".pdf";
        ...
        public string DownloadOneFile(List<int> ids, string SaveFolderPath, List<string> selectedPages, string RootPath, int entityId = 0)
        {
            ...
            string TemplatePathFile = Path.Combine(RootPath, "Forms", AppConstants.Form1098FTemplateFileName);
        ...
        public async Task<MessageResponseModel> DeletePermeant(int id)
        {
            ...
            var recordToDelete = _evolvedtaxContext.Tbl_1098_F.First(p => p.Id == id);
            if (recordToDelete != null)
            {
                _evolvedtaxContext.Tbl_1098_F.Remove(recordToDelete);
                await _evolvedtaxContext.SaveChangesAsync();
        ...
        public async Task<MessageResponseModel> KeepRecord(int id)
        {
            ...
            var recordToUpdate = _evolvedtaxContext.Tbl_1098_F.First(p => p.Id == id);
